...
import { Search, Edit, Trash2, Download, Star, Package, IndianRupee, TrendingUp, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ProductsPage: React.FC = () => {
    ...
    const categories: string[] = ['Electronics', 'Wearables', 'Photography', 'Gaming', 'Accessories'];
    ...
    const handleSubmit = (): void => {
        ...
        const productData: Omit<Product, 'id'> = {
            ...
            status: stockValue === 0 ? 'out_of_stock' :
                stockValue < 20 ? 'low_stock' : 'active'
        };
                        ...
                        <table className="w-full">
                            <thead className="">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-800">Sales</th>
                            ...
                            <tbody className="divide-y border">
                                {currentProducts.map((product) => (
                                    <tr key={product.id} className=" hover:bg-gray-100 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`font-semibold ${product.stock < 20 ? 'text-yellow-400' : 'text-black'}`}>
                                                {product.stock}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-3 py-1 rounded-full text-sm border ${getStatusColor(product.status)}`}>
